...
/// row's own label text against the registry's account names rather than by fixed row number. The
/// registry (see <see cref="InvestmentAccountMigrator"/>) must already be seeded before this runs.
/// A handful of older sheets use a label that no longer matches any current account Name (a
/// pre-rename label, or a spreadsheet typo) — <see cref="LegacyAccountLabels"/> maps those straight
/// to the account's current Name; this mapping is an import-only concern and intentionally isn't
/// modeled on the account entity itself. Rows whose label matches no known name or legacy label are
/// simply not written — no fabricated snapshot is ever created. For a matched row, every one of the
/// 12 months gets a snapshot: a genuinely blank cell becomes an explicit 0 (so an account's
/// existence in a year is derivable purely from snapshot presence afterward), while a non-blank
/// cell that fails to parse is flagged on the report and left without a snapshot for that month.
/// This is also the only place <see cref="InvestmentSnapshot"/> data is sourced from, so it both
...
public static class ResumoValidationReader
{
    ...
    /// <summary>Older Resumo-sheet labels that don't match any current account Name, mapped to the
    /// Name of the account they refer to: "Chip Cash ISA" predates the "Gleison" suffix added once
    /// Ariana's own Chip Cash ISA existed to disambiguate from, and "Instant ISE Issue 1" is a
    /// spreadsheet typo of "Instant ISA Issue 1" that appears in some years.</summary>
    private static readonly IReadOnlyDictionary<string, string> LegacyAccountLabels = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
    {
        ["Chip Cash ISA"] = "Chip Cash ISA Gleison",
        ["Instant ISE Issue 1"] = "Instant ISA Issue 1"
    };
    ...
    private static bool TryResolveAccount(string rawLabel, IReadOnlyCollection<InvestmentAccount> accounts, out InvestmentAccount? account)
    {
        ...
        var accountName = LegacyAccountLabels.TryGetValue(normalized, out var mapped) ? mapped : normalized;
