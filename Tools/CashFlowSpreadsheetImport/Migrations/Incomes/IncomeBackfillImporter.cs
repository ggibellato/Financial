...
/// <summary>
/// Backfills historical Income entries from each monthly sheet's income-totals area (see
/// <see cref="IncomeTotalsReader"/>). One entry is created per month per income source, dated the
/// first of that month, skipping months where a source's total is zero. Every entry is
...
public static class IncomeBackfillImporter
{
    ...
    private static bool TryImport(CashFlowData data, DateOnly date, IncomeTotal total)
    {
        ...
        var incomeSource = data.IncomeSources.FirstOrDefault(s => string.Equals(s.Name, total.Source, StringComparison.OrdinalIgnoreCase));
        var bank = data.Banks.FirstOrDefault(b => string.Equals(b.Name, TargetBankName, StringComparison.OrdinalIgnoreCase));
        if (incomeSource is null || bank is null)
        {
            return false;
        }
