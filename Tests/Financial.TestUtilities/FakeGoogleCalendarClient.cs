...
/// <summary>Hand-written IGoogleCalendarClient test double - no live Google network calls,
/// matching this codebase's "test the wiring, not the live SDK" convention for Google
/// integrations. Configure the public fields, then read the *CallCount / *Calls lists back.</summary>
public sealed class FakeGoogleCalendarClient : IGoogleCalendarClient
{
    ...
    public GoogleCalendarTokenResult ExchangeResult { get; set; } = new("access-token", "refresh-token", DateTimeOffset.UtcNow.AddHours(1));
    public GoogleCalendarTokenResult? RefreshResult { get; set; }
    ...
    public Task<GoogleCalendarTokenResult> ExchangeCodeForTokenAsync(string code, CancellationToken cancellationToken = default)
    {
        ExchangeCallCount++;
        return Task.FromResult(ExchangeResult);
    }

    public Task<GoogleCalendarTokenResult> RefreshAccessTokenAsync(string refreshToken, CancellationToken cancellationToken = default)
    {
        RefreshCallCount++;
        if (RefreshThrowsRevoked)
        {
            throw new GoogleCalendarTokenRevokedException("Refresh token was revoked.");
        }
