...
public class ExpenseServiceTests
{
    ...
    {
        Action act = () => new ExpenseService(null!, Tracer, Logger);
    ...
    {
        Action act = () => new ExpenseService(new StubCashFlowRepository(), null!, Logger);
    ...
    public async Task AddExpenseAsync_WithValidRequest_RecordsSuccessfulSpan()
    {
        ...
        var service = new ExpenseService(repository, tracer, Logger);
    ...
    public async Task UpdateExpenseAsync_WithExistingId_UpdatesInPlace()
    {
        ...
        var updateRequest = new ExpenseUpdateDTO
        {
            ...
            Description = "Updated",
            Value = 10m,
            CategoryId = ResolveCategoryId(repository, "Casa"),
    ...
    private static ExpenseCreateDTO ToCreateDto(StubCashFlowRepository repository, ExpenseCreateRequest r) => new()
    {
        ...
        Value = r.Value,
        CategoryId = ResolveCategoryId(repository, r.Category),
    ...
    /// <summary>An unresolvable name maps to a random, never-seeded Guid so tests exercising an unrecognized reference still hit the "not found" path rather than the "omitted" path.</summary>
    private static Guid ResolveCategoryId(StubCashFlowRepository repository, string categoryName) =>
        repository.Categories.FirstOrDefault(c => c.Name == categoryName)?.Id ?? Guid.NewGuid();
    ...
    {
        Action act = () => new ExpenseService(new StubCashFlowRepository(), Tracer, null!);
    ...
    public async Task AddExpenseAsync_LogsEntryAndSuccess_WithOperationNameOnly()
    {
        ...
        var service = new ExpenseService(repository, new RecordingTelemetryTracer(), logger);
        var request = ToCreateDto(repository, ValidCreateRequest());
