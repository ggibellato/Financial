...
public class MonthlyExpenseSheetImporterTests : IDisposable
{
    ...
    public void Import_2017ShapedSheet_QuemThenMotivo_ParsesExpensesCorrectly()
    {
        ...
        var expenses = MonthlyExpenseSheetImporter.Import(sheet, 2017, 2, Today, _report, Banks, CreditCards, Categories);
    ...
    public void Import_2026ShapedSheet_MotivoThenQuem_ParsesExpensesCorrectly()
    {
        ...
        var expenses = MonthlyExpenseSheetImporter.Import(sheet, 2026, 7, Today, _report, Banks, CreditCards, Categories);
    ...
    public void Import_UnrecognizedCategory_SkipsExpenseAndFlagsRow()
    {
        ...
        _report.RowIssues.Should().ContainSingle(i => i.RawValue == "TotallyUnknownCategory" && i.SheetName == "Out2017");
    }
    ...
    public void Import_DayDoesNotExistInMonth_ClampsToLastValidDayAndFlagsRowWithoutDroppingTheExpense()
    {
        ...
        var expenses = MonthlyExpenseSheetImporter.Import(sheet, 2026, 2, Today, _report, Banks, CreditCards, Categories);

        expenses.Should().ContainSingle();
        expenses[0].Date.Should().Be(new DateOnly(2026, 2, 28));
        expenses[0].Value.Should().Be(130.0m);
        _report.RowIssues.Should().ContainSingle(i => i.SheetName == "Fev2026" && i.RawValue == "29");
    }

    [Fact]
    public void Import_ValueEnteredAsTextWithCommaDecimalSeparator_ParsesAsDecimalNotInflated100x()
    {
        var sheet = _workbook.AddWorksheet("Mar2017");
        sheet.Cell(1, 1).Value = "Dia";
        sheet.Cell(1, 2).Value = "Motivo";
        sheet.Cell(1, 3).Value = "Quem";
        sheet.Cell(1, 4).Value = "Valor";

        // Confirmed real-_workbook data quality issue (Mar2017, cell D82): the value was entered as
        ...
        var expenses = MonthlyExpenseSheetImporter.Import(sheet, 2017, 3, Today, _report, Banks, CreditCards, Categories);
    ...
    {
        var sheet = _workbook.AddWorksheet("Ago2026");
        WriteExpenseRow(sheet, row: 150, paymentSourceTag: null);

        var expenses = MonthlyExpenseSheetImporter.Import(sheet, 2026, 8, Today, _report, Banks, CreditCards, Categories);
    ...
    {
        var sheet = _workbook.AddWorksheet("Set2026");
        WriteExpenseRow(sheet, row: 129, paymentSourceTag: null);

        var expenses = MonthlyExpenseSheetImporter.Import(sheet, 2026, 9, Today, _report, Banks, CreditCards, Categories);
    ...
    public void Import_RowResolvesToACardNameWithNoMatchingSeededEntity_FlagsRowWithRowNumberAndCardNameAndSkipsExpense()
    {
        ...
        var expenses = MonthlyExpenseSheetImporter.Import(sheet, 2026, 7, Today, _report, Banks, creditCardsMissingBaAmex, Categories);

        expenses.Should().BeEmpty();
        _report.RowIssues.Should().ContainSingle(i =>
