...
- Excluded: per-transaction credit-card attribution (the source data has none — see Technical Decisions); recurring/scheduled sync (this is a single manual run); any UI.
...
| Credit-card per-transaction attribution | Not imported — no `Expense.CardTag` is set from historical rows, and no `CardStatement` rows are pre-populated; F04's existing lazy generation creates all 5 as unpaid/zero the first time any imported month's card view is opened | Approximate an attribution using the monthly J-K "Ajuste" aggregate figure | The source spreadsheet explicitly never records which of the 5 cards a given expense used at the transaction level (confirmed in the project's own context notes) — only a monthly aggregate adjustment figure exists, which cannot be decomposed back into a specific card or specific expense without fabrication. Leaving this honestly empty (matching the Mensais decision's "no fabricated history" principle) is consistent and correct. |
