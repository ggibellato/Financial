...
### F01. Dashboard Aggregate
- [ ] Dashboard market value, invested, unrealised and realised gain, income YTD/lifetime and both
      XIRR figures reconcile exactly with the sum of the equivalent per-broker figures
- [ ] `IsPartial` is true and an inline notice appears whenever at least one Active holding has no
      market value
- [ ] Realised gain/loss excludes every `Superseded` `DisposalRecord`
- [ ] Income YTD resets to 0 on a new calendar year and lifetime income never does
...
- [ ] A holding with a detected monthly/quarterly/four-monthly frequency shows a projected entry dated
      exactly one interval after its last credit
- [ ] A holding with no detectable frequency is omitted from the list
- [ ] Changing the window filter (30/90/180 days) changes exactly which projected entries are shown,
      with no change to the projected dates themselves
- [ ] The empty state shows when no holding has a projection inside the selected window
