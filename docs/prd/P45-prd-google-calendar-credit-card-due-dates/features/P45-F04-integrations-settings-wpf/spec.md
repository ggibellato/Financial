...
**Scope:**
- Included: the new "Integrations" entry under the existing WPF "Settings" nav category, the connection panel (all states from PRD Experience), the per-card DataGrid with per-row Retry, the Disconnect confirmation (reusing the existing `IDialogService.Confirm`), the new `IBrowserLauncher` abstraction, and a self-hosted OAuth loopback listener (`ICalendarOAuthCallbackListener`) - see the 2026-09-08 addendum below.
- Excluded: any change to F01/F02's Application/Infrastructure services (consumed as-is); the OAuth consent screen itself (served by Google).

> **2026-09-08 addendum:** the original "Detecting connect completion" and "OAuth callback reachability" decisions below (poll on a `DispatcherTimer`, redirect at `Financial.Api`) were superseded after real-world use showed WPF's connect flow breaking with `ERR_CONNECTION_REFUSED` whenever `Financial.Api` wasn't already running - `Financial.App` has no HTTP server of its own, so it had no way to receive Google's redirect independently. `Financial.App` now hosts its own OAuth loopback listener (`ICalendarOAuthCallbackListener`, `System.Net.HttpListener`-based, Google's documented loopback pattern for native/desktop apps), configured with its own `CashFlow:GoogleCalendar:RedirectUri` distinct from `Financial.Api`'s (both registered against the same Google Cloud Console OAuth client). `Connect()` now awaits the listener directly and calls `ICalendarIntegrationService.CompleteConnectionAsync` in-process - the same method `Financial.Api`'s controller was already calling - so there is no more status-polling loop and no dependency on `Financial.Api` being started. The rows below are kept for historical context; treat the addendum as authoritative.
>
> **Testing note:** `CalendarOAuthCallbackListenerTests` covers the listener's query-parsing/redirect-extraction logic and its cancellation behaviour as fast unit tests (its parsing helpers are `internal`, matching `GoogleCalendarOAuthClient.BuildEvent`'s existing precedent for test-only visibility). A true end-to-end run (a real socket bind, a real HTTP client hitting it) was deliberately left out of the automated suite: on the machine this was built on, every such loopback connection was refused outright by local security software inspecting the freshly-built test executable, with no reliable way to tell "environment is blocking this" from "the code is broken" in CI-portable test code - the same class of gap `docs/rules/implementation.md`'s testing guide already accepts for live web-scraper selectors. Verified manually instead by running `Financial.App` and completing a real Google Calendar connect end to end.
