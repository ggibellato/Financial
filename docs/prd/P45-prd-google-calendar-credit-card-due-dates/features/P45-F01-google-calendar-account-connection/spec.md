...
| `Tests/Financial.Api.Tests/Acceptance/GoogleCalendarAccountConnectionAcceptanceTests.cs` | Integration (AC-tracing) | All 4 endpoints via `ApiEndpointTests`, real service/store, only `ICalendarProvider` faked | One `[Trait("AC", …)]` test per §9 criterion |
