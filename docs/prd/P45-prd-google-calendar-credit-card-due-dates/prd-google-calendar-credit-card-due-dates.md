...
**Capabilities:**
- OAuth 2.0 authorization-code flow against Google, requesting the `https://www.googleapis.com/auth/calendar` scope (full calendar management is required to create and later delete the dedicated calendar, not just its events) plus `https://www.googleapis.com/auth/userinfo.email` (required for the connected-account-email lookup below; the calendar scope alone does not grant access to Google's userinfo endpoint).
- Google OAuth client ID, client secret, and redirect URI are read from configuration (`CashFlow:GoogleCalendar:ClientId` / `ClientSecret` / `RedirectUri`), following the same per-provider configuration convention already used for `GoogleDrive:CredentialsPath`. Since `Financial.Api` and `Financial.App` each read their own `appsettings.*.json`, they are configured with two distinct redirect URIs registered against the same Google Cloud Console OAuth client: the API's targets a provider-agnostic API endpoint, `{API_BASE_URL}/api/v1/financial/integrations/calendar/callback`; WPF's targets its own self-hosted loopback listener (see F04), a fixed local port such as `http://localhost:8082/`.
- On successful consent, the app looks for an existing calendar named "Financial - Credit Card Due Dates" in the connected account and reuses it if found - otherwise it creates one. Reuse matters because disconnecting (below) deliberately leaves the dedicated calendar and its events behind rather than deleting them; without reuse, every disconnect-then-reconnect cycle on the same account would leave an orphaned duplicate calendar. Either way, the app persists the access token, refresh token, the calendar's ID, connected account email, and connected-at timestamp to a dedicated local credentials file (path from `CashFlow:GoogleCalendar:CredentialsPath`, mirroring the `GoogleDrive:CredentialsPath` pattern), separate from `data-cashflow.json`. This file is added to `.gitignore`, matching every other local credentials/data file in the repo. Reusing a calendar does not attempt to recover which of its events (if any) already correspond to which credit card - the local event-id mapping always starts empty on a fresh connection, so a subsequent sync creates new events for cards it doesn't already have a mapping for, the same way it would for a brand-new calendar.
...
- Disconnecting revokes the token with Google's token-revocation endpoint and deletes the local credentials file, but deliberately leaves the dedicated calendar and every event in it untouched in the user's Google account — the user may still want that data even after telling the app to stop managing it. (Connecting a *different* account while one is already connected is the one case that still deletes the outgoing account's dedicated calendar first — see above - since that calendar would otherwise become orphaned, unreachable through the app.) Disconnect always removes local state even if the remote revoke call fails, so the app never reports "connected" when it can no longer act on the calendar.
- `GET /api/v1/financial/integrations/calendar/status` exposes the current connection status for both front ends to poll.

**Experience:**
- Connect (Web): user clicks "Connect Google Calendar"; the browser navigates to the Google consent screen directly. After the user grants access, Google redirects to the API's callback endpoint, which completes the exchange, creates the calendar, and redirects/responds with a success page telling the user to return to the app; the Web page polls `/status` on window focus regaining after the redirect so the UI updates without a manual refresh.
- Connect (WPF): WPF has no HTTP server of its own and cannot depend on `Financial.Api` running separately, so it opens the consent URL in the OS default browser and, in parallel, briefly hosts its own OAuth loopback listener (`System.Net.HttpListener` bound to a fixed local port, the standard pattern Google documents for native/desktop apps) to receive the redirect directly. Once the redirect arrives, WPF completes the exchange in-process via the same Application service `Financial.Api`'s controller calls, without any HTTP round-trip, and updates its own status immediately — no polling needed.
- Disconnect always requires an explicit confirmation step (see F03/F04) before the call is made — it stops the app managing the calendar and revokes its access, though the calendar and its events remain in the user's Google account afterward.
...
- Disconnect's remote token-revocation call fails → local connection state and credentials file are still cleared, and the user is shown a one-time notice: "Disconnected in the app. You may need to manually revoke access in your Google Account if this happens again."
...
- Reconciliation: when a sync has no local mapping for a card (a fresh connection always starts with an empty mapping - including when F01 reused an existing calendar rather than creating a new one), it first checks whether the calendar already has an event whose title starts with that card's name before creating one. A match is adopted (its id is recorded, its date/content updated) instead of creating a duplicate; only when no match exists does the app create a new event. This is what makes calendar reuse (F01) safe against duplicating events across a disconnect-then-reconnect cycle.
...
- **Disconnect confirmation:** clicking "Disconnect" opens a confirmation dialog explaining that the app will stop managing due-date events and revoke its access, but that the dedicated calendar and its events remain in the user's Google account; only confirming proceeds.
...
**Experience:**
- Connect: button opens the consent URL via the OS default browser (no embedded WebView2) and, in the same action, starts a self-hosted OAuth loopback listener (see F01) that receives Google's redirect directly — no dependency on `Financial.Api` running separately. Once the redirect arrives, the connection completes in-process immediately; there is no status-polling loop. A 5-minute timeout surfaces an inline error if the user abandons the browser flow.
...
- [x] **P45-F01-google-calendar-account-connection-03** Disconnecting revokes the token, deletes the local credentials file, and `/status` reports `connected: false` immediately after - while leaving the dedicated calendar and its events untouched in Google Calendar.
...
- [x] **P45-F04-integrations-settings-wpf-02** Clicking "Connect" opens the system default browser to the Google consent URL; no embedded browser control is used.
- [x] **P45-F04-integrations-settings-wpf-03** The view's status updates to "connected" after the user completes consent in the browser and returns to the app, without requiring an explicit manual refresh action.
...
- [x] **P45-F04-integrations-settings-wpf-05** Disconnect requires the same explicit confirmation as F03 before proceeding.
