...
- **Excluded (later waves/features):**
  - Changing `CardStatementService`'s settlement **matching key** from `Date.Year/Month` to `InvoiceDate.Year/Month` (F02).
...
| Application | `Financial.CashFlow.Application/Services/CardStatementService.cs` | Rollback snapshot in `UnmarkStatementPaidAsync` reads `Date` instead of the removed `SettledAt` |
...
| `Financial.CashFlow.Application/Services/ExpenseService.cs` | Modified | Expense use cases | Remove `SettledAt` from the `ToDto` mapping |
