...
| `Services/AnnualSummaryService.cs` | Modified | Annual investment diff computation | `GetInvestmentDiffsForYear` resolves the year-scoped account list via the new rule and builds `InvestmentAccountAnnualDiffDTO` rows (and the `NetPosition` sum) only over those accounts |

No Infrastructure, Presentation, or frontend files change.

## 5. API Contracts

No request/response shape changes to either existing endpoint (`GET /api/v1/financial/investment-snapshots/{year}/{month}`, `GET` annual-summary investments). Both return the same DTO shapes as before — just a year-dependent subset of rows instead of always all 19.
...
| `Tests/Financial.CashFlow.Application.Tests/Services/AnnualSummaryServiceTests.cs` | Unit | `AnnualSummaryService` | Updated: `GetInvestmentDiffsForYear` for a past year returns only accounts present that year; for the current year returns only active accounts; `NetPosition` reflects only the scoped accounts |
...
| `GetInvestmentDiffsForYear_PastYear_ReturnsOnlyAccountsPresentThatYear` (`AnnualSummaryServiceTests`) | Past year, snapshots for a subset of accounts | `Accounts` contains only that subset |
| `GetInvestmentDiffsForYear_CurrentYear_ExcludesDisabledAccounts` (`AnnualSummaryServiceTests`) | Current year, mix of active/disabled accounts, some with stray snapshots | `Accounts` contains only active ones |
| `GetInvestmentDiffsForYear_NetPositionSumsOnlyScopedAccounts` (`AnnualSummaryServiceTests`) | Past year, one in-scope and one out-of-scope account with values | `NetPosition.MonthlyValues` reflects only the in-scope account |
