...
| New exception for delete guarded by cross-entity references | A single `EntityInUseException(string message)` in `Financial.CashFlow.Application.Exceptions`, mapped once to 409 in the middleware, reused by F06 (Category), F07 (Credit Card), F08 (Income Source) | A `BankInUseException` specific to this feature | The PRD applies the identical guard shape to four entities (Section 6, F05-F08); one exception type avoids four near-identical classes and four near-identical middleware `catch` blocks. Documented here since F05 is first to need it |
