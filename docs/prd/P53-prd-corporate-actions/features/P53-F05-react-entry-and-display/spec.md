...
**Primary and secondary actions:** Primary: "New Corporate Action" (opens the inline form) and, once open, the type-specific save action ("Add Split"/"Add Merger"/"Add Spin-off", following the trigger→title→confirm naming-consistency rule — see §4 decision on button labels). Secondary: per-row Edit/Delete icon buttons in the history table's trailing Actions column (unlike `DisposalsTab`'s read-only chain-history chevron, Corporate Actions are user-editable/deletable per F04, so the pattern to follow is `TransactionsTab`'s Edit/Delete pair, not `DisposalsTab`'s).
