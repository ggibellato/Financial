...
**The Annual Summary page shows too much at once**
...
- There is no way to focus on one kind of annual information without scrolling past the other two tables

**Income and category data are artificially split**
- Salary, Salary After Taxes, and Dividendo/Juros live in a separate "Income Summary" table below Category Totals, even though reviewing annual financial results means looking at income and expenses together
...
- There is no per-row Average column, so seeing a "typical month" value requires mentally dividing the Annual Total by 12
...
- Too much shown at once → the new sub-tab shell isolates Category Totals and Investments into separate views, so each visit shows only the relevant kind of annual information
...
- The sole user and maintainer of this self-hosted personal finance tool, already familiar with every category, account, and figure on today's Annual Summary tab
- Reviews annual totals periodically to check spending trends, income totals, and investment account progress across a full calendar year
...
- **Eliminate** manual calculation of annual derived figures by adding Total despesas, Resultado (R-D-Inv), a per-row Average column, and the three new investment summary figures (Year Progress, Average Month Result, Sum of Month Results) directly to the page.
...
- **Preserve** all existing figures and data without regression — every category, account, and income figure shown today remains available and numerically unchanged after the split.
  Metric: 100% of existing Annual Summary test assertions pass once relocated to their respective sub-tab, with zero numeric regressions.
...
### F01. Annual Summary Navigation Shell
- As the developer, I want a single year picker at the top of the Annual Summary tab that applies to both sub-tabs so I never have to re-select the year when switching views
- As the developer, I want two clearly labeled sub-tabs — Category Totals, Investments — so I can focus on one kind of annual information at a time
- As the developer, I want the Category Totals sub-tab selected by default whenever I open the Annual Summary tab
...
- As the developer, I want an Average column next to the Annual Total column on every row so I can see a typical month's value at a glance without doing the division myself
...
- Default active sub-tab on every entry to the Annual Summary tab (initial load or navigating back from another CashFlow view) is Category Totals
...
**Experience:**
- Developer opens the Annual Summary tab → sees the year picker at top, the two sub-tab buttons below it (Category Totals visually highlighted as active), and Category Totals content beneath
...
- Each row has 12 month columns (Jan–Dec), followed by an Average column, followed by a Annual Total column
...
- The 14 category rows use the existing `CategoryAnnualTotalDTO.MonthlyTotals` per category, unchanged from today's values
...
- Annual Total column: for rows sourced directly from the API (Salary, Salary After Taxes, Tax Difference, Dividendo/Juros, each category) uses the API's existing annual total field; for the two new derived rows (Total despesas, Resultado) it is the sum of that row's 12 monthly values
- All monthly, average, and annual-total figures use the existing 2-decimal numeric formatting (`formatN2`) used throughout the Annual Summary tab today
...
- Resultado and Total despesas rows are visually emphasized (bold), consistent with the existing annual-total emphasis styling used elsewhere on this tab
...
- A "Total" row follows the 11 account rows, showing the net position for each of the 12 months using the existing `NetPositionAnnualDiffDTO.MonthlyValues` (assets minus liabilities, already computed server-side)
- A "Month Result" row follows the Total row, showing the existing `NetPositionAnnualDiffDTO.MonthlyDiffs` (11 values, Feb through Dec month-over-month change of the Total row); the January column under Month Result is empty, since there is no prior month to diff against
- Three summary figures render below the table, in this order: Year Progress (existing `NetPositionAnnualDiffDTO.FullYearNetChange`, i.e. December's total minus January's), Average Month Result (arithmetic mean of the 11 Month Result values), Sum of Month Results (sum of the 11 Month Result values)
...
- All monthly and summary figures use the existing 2-decimal numeric formatting (`formatN2`) used throughout the Annual Summary tab today
...
- No changes to any Annual Summary API endpoint or DTO — all three existing endpoints (`expense-categories`, `investment-diffs`, `income-summary`) remain unchanged; every new figure (Resultado, Total despesas, per-row Average, Month Result row reuse, Year Progress, Average/Sum of Month Results) is computed client-side from data already returned today
...
- No persistence of the active sub-tab across page reloads or navigation away from and back to the Annual Summary tab — it always resets to Category Totals
...
|---|---------|----------|--------------|
| F01 | Annual Summary Navigation Shell | 1 | None |
...
### F01. Annual Summary Navigation Shell
- [x] Opening the Annual Summary tab shows the year picker, the two sub-tab buttons (Category Totals, Investments), and Category Totals content by default
...
- [x] Salary, Salary After Taxes, Tax Difference, Dividendo/Juros, and each category row's monthly and annual-total values match the values shown before the redesign for the same year
...
- [x] Every row displays an Average column equal to the arithmetic mean of its 12 monthly values, positioned between the Dec column and the Annual Total column
- [x] Total despesas and Resultado rows render visually emphasized (bold), consistent with existing annual-total styling
