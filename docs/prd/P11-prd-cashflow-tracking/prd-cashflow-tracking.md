...
### F03. Monthly Expense Tracking
- [x] An expense with a valid date, description, value, category, and payment source saves and appears in that month's category total
- [x] Saving an expense with a zero value or missing category is rejected with a validation message
- [x] Editing or deleting an existing expense updates that month's category totals accordingly

### F04. Credit Card Charge Tracking & Statement Reconciliation
- [x] An expense tagged to a card counts toward its category total immediately but does not reduce the bank balance until the statement is marked paid
- [x] Marking a card's statement paid zeroes its outstanding total and reduces the combined adjustment figure by the same amount
- [x] Marking a statement paid when the outstanding total is already zero is a no-op with a confirmation message, not an error

### F05. Reserva Reserve Pool & Automated Split
- [x] Entering Salario Gleison, Salario Ariana (net-of-tax), Lottery, and Dividendo/Juros for a month computes Dizimo as exactly 10% of (combined net-of-tax salary + gross Lottery + gross Dividendo/Juros)
- [x] The Limpo pool (combined net-of-tax salary minus Dizimo) splits into exactly 1/3 Investimento, 1/3 house-treats, 1/6 Ariana, 1/6 Gleison — Lottery and Dividendo/Juros contribute only to Dizimo, not to this split
- [x] A negative value for any of the four income fields is rejected before any bucket is touched
...
### F10. Historical Spreadsheet Import
- [x] Every one of the 115 monthly tabs (February 2017-2026) is either successfully imported or reported with a specific per-sheet error, with none silently skipped
- [x] No tab from `Julho 2014` through `Janeiro 2017` is read or imported by this feature
...
- [x] Re-running the import against a freshly emptied `data-cashflow.json` produces the same result as the first run
...
- [x] An expense can be created, edited, and deleted from this view
- [x] A card statement can be marked paid from this view
...
- [x] Expense data written through F02's storage abstraction (F01/F02) is correctly read back by F03 as categorized expense records
