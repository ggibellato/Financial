...
| Re-fetching buckets on every mutation-triggered refresh | `RefreshAsync(bool includeBuckets = true)`; the four mutation success paths (split/withdrawal/edit/delete) pass `includeBuckets: false` | Keep buckets in every `RefreshAsync` call unconditionally, matching `GetBucketBalances()`/`GetMovementHistory()`'s always-refetch pattern | Originally scoped as a pure efficiency question (an in-process call is cheap, unlike F06's web HTTP round-trip) and left unoptimized — but `/code-review` found a real correctness bug: `ReplaceAll(Buckets, ...)` clears the collection before re-adding items, and the movement grid's row actions aren't gated behind the Withdrawal/Edit form being closed, so a mutation elsewhere on the page could silently null out an open form's `SelectedValue`-bound bucket via WPF's `Selector` reset-on-clear behavior. `includeBuckets: false` on mutation refreshes avoids ever touching `Buckets` outside initial load/retry, where no bucket form can be open yet |
