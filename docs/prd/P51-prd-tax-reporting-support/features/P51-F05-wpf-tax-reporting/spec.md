...
| `Financial.App/Views/Investment/PortfolioSummaryView.xaml` | Modified | Display it next to Country | New `TextBlock` pair inserted immediately after the existing ISIN/Country row in `AssetSummaryTemplate`, row indices renumbered accordingly |
