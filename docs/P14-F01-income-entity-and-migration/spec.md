...
- Excluded: any frontend UI (F04); the tithe calculation (F03); the Incoming card (F05); bank balance changes (F06); the Annual Summary table (F07); historical income backfill (out of scope per PRD).
...
- "F07 correctly aggregates F01's income data across all 12 months" — depends on `GetIncomes()` exposing `GrossValue`/`NetValue`/`IncomeSource` correctly across an arbitrary date range, covered here by `IncomeServiceTests` and `CashFlowDataTests`; the annual aggregation itself is verified in F07's own spec
