...
| Average Month Result / Sum of Month Results (frontend) | Filter out `null` before averaging/summing, so these figures include January whenever it has a real value (every year except the earliest) | Keep them Feb-Dec-only forever, ignoring the new January value entirely | The PRD's whole point is that January should behave like a normal month once real data exists for it; artificially excluding it from these already-existing summary statistics once it's real would be an inconsistency the PRD doesn't call for. Not explicitly stated in the PRD's Capabilities, so documented here as an interpretation, not literal spec text — genuinely reasonable either way, but this is the more internally consistent choice given "Month Result" already means "this row, whatever months it has." |
