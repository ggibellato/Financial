...
**What stays untested (documented, not a gap):** `GoogleFinanceService.GetAssetValue`'s two successful branches (the live `GoogleFinance.GetFinancialInfoSnapshot`/`GetCryptocurrencyFinancialInfoSnapshot` calls) have no unit seam to intercept — the same limitation already documented for this exact code path in `docs/P08-F01-asset-snapshot-fetcher-contract-standard-fetcher/spec.md`. Verified by code review that each branch is a direct, unmodified pass-through of `(exchange, ticker)` or `(currency, ticker)`. The DI registration line itself is also not unit-tested, consistent with this codebase's existing convention.
