...
const TableSelectableRow: React.FC<TableSelectableRowProps> = (props) => {
  ...
  const onClick = React.useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => toggleRow(e, rowId),
    [toggleRow, rowId]
  );
...
export const PrimaryCell = (): JSXElement => {
  return (
    <Table
      aria-label="Table with primary cell layout"
      style={{ minWidth: "500px" }}
    >
      <TableHeader>
        <TableRow>
...
export const ResizableColumnsControlled = (): JSXElement => {
  const [columns, setColumns] =
    React.useState<TableColumnDefinition<Item>[]>(columnsDef);
...
export const SingleSelectControlled = (): JSXElement => {
  ...
  return (
    <Table
      aria-label="Table with controlled single selection"
      style={{ minWidth: "550px" }}
    >
      <TableHeader>
        <TableRow>
