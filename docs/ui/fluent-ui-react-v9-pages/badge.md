...
| `as` | `"div"` | No |  |  |
| `appearance` | `"outline" "filled" "ghost" "tint"` | No |  | A Badge can be filled, outline, ghost, inverted @defaultvalue filled |
| `color` | `"subtle" "brand" "danger" "important" "informative" "severe" "success" "warning"` | No |  | A Badge can be one of preset colors @defaultvalue brand |
| `iconPosition` | `"before" "after"` | No |  | A Badge can position the icon before or after the content. @defaultvalue before |
| `shape` | `"rounded" "circular" "square"` | No |  | A Badge can be square, circular or rounded. @defaultvalue circular |
...
`ghost-subtle` and `outline-subtle` are intended only for use on a branded background — the boxed "On brand background:" examples above demonstrate the intended usage context.
