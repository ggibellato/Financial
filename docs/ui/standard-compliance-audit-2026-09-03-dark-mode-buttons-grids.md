...
| 6 | ~~Web: `ExpenseForm.tsx:212`, `IncomeForm.tsx:149`, `MensaisPage.tsx:344,398`, `ControleMaePage.tsx:240`, `InvestmentSnapshotsPage.tsx:117`, `TransactionsTab.tsx:194`, `CreditsTab.tsx:183`, `PriceHistoryTab.tsx:157` (8 files, 9 message bars). WPF: 10 ViewModels' `*SaveError`/`*FieldError` pairs bound into `AddBillFormView.xaml`, `EditBillFormView.xaml`, `CreateEntryFormView.xaml`, `EditEntryFormView.xaml`, `EditReserveMovementFormView.xaml`, `EditSnapshotValueFormView.xaml`, `ExpenseFormView.xaml`, `IncomeFormView.xaml`, `IncomeSplitFormView.xaml`, `TransferFormView.xaml`, `WithdrawalFormView.xaml`, `BalanceAdjustmentFormView.xaml` (12 files)~~ | **Reported by the user, resolved 2026-09-04.** A field-specific validation error rendered twice: once inline under its `Field`/`FieldErrorTextStyle` (correct), and again in the generic bottom-of-form `MessageBar`/`TextBlock`, which showed the raw error string unconditionally. `TransferForm.tsx` (Web) already had the right pattern (`generalError = saveErrorField === null ? saveError : null`) — every other form with per-field errors, on both platforms, was missing it. Fixed by applying that same guard everywhere: Web's `MessageBar`s now check `xErrorField === null` before rendering; WPF's ViewModels each gained a `*GeneralSaveError` computed property (null whenever any of that form's `*FieldError` properties claimed the message) and the bottom-of-form `TextBlock`s now bind to it instead of the raw `*SaveError`. The WPF `TargetBalanceFieldError`/etc. XML doc comments describing this duplication as "additive… existing bottom-of-form message" reflected the original (now corrected) design intent, not a bug in the code — the user's report is what changed the requirement. **The second half of the report — "the field with the error message gets out of alignment with other fields, it does move up" — is not yet root-caused.** Best code-grounded theory: `FieldErrorTextStyle` (`App.xaml:170-181`) sets `Visibility` to `Collapsed` (not `Hidden`) when a field has no error, so a `Grid.RowDefinition Height="Auto"` row's height changes every time a sibling field's error toggles on/off, shifting whatever sits below it — but this shifts *later* rows, not the erroring field itself, so it may not be the full story. Needs the user's visual confirmation after this fix (removing the duplicate message may already have reduced the shift) before deciding whether a further layout fix (e.g. reserving space with `Hidden` instead of `Collapsed`) is warranted. | Medium (High if the layout shift is confirmed separately) |
