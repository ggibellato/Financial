...
| Balance Correction (`useBalanceAdjustmentForm.ts` / `AdjustmentWorkflowViewModel.cs`) | Today already, every time, both platforms | `bankName`/`AdjustmentFormBankName` — **resolved (P38-F10 spec, Decision D4):** confirmed with the user that the bank *is* persisted, matching every other form; this reverses Web's prior "Create opens with no bank pre-selected" deliberate-design comment (`useBalanceAdjustmentForm.ts:62-65`, since updated) | Target balance, note |
