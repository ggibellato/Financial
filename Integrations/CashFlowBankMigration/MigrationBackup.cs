...
public static class MigrationBackup
{
    ...
    public static string Create(string dataPath)
    {
        ...
        var backupPath = Path.Combine(directory, $"{name}.backup-bank-migration-{timestamp}{extension}");

        var suffix = 1;
        while (File.Exists(backupPath))
        {
            backupPath = Path.Combine(directory, $"{name}.backup-bank-migration-{timestamp}-{suffix}{extension}");
            suffix++;
        }
