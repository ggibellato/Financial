...
namespace Financial.CashFlow.Infrastructure.Integrations.CashFlowIncomeMigration.SpreadsheetImport;
...
/// CashFlowSpreadsheetImport project - each source's row is located by matching its label text
/// rather than by a fixed row/column. Whatever numeric value(s) sit immediately to the right of a
/// matched label are read: a single value is the net total; two values are gross then net. The
/// "Dizimo" row is intentionally never matched here - it is tithe, not an <see cref="IncomeSource"/>.
/// </summary>
