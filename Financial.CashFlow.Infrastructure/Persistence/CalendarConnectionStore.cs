...
/// <summary>
/// Reads/writes the local calendar-connection credentials file - separate from
/// <c>data-cashflow.json</c>, so it is never loaded/saved through <c>ICashFlowRepository</c>.
/// Provider-agnostic: it persists whichever <see cref="ICalendarProvider"/> connection is active.
...
/// </summary>
public sealed class CalendarConnectionStore : ICalendarConnectionStore
{
    ...
    public CalendarConnectionStore(string? filePath)
    {
    ...
    public CalendarConnection? Load()
    {
        ...
        return JsonSerializer.Deserialize<CalendarConnection>(json, SerializerOptions);
    }
