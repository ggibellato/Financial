...
- [ ] T050 [P] Add `ILogger`-based use-case entry/success logging alongside the spans added in T029-T031/T033-T034 (logging-audit.md priority 2 — the two land together naturally at the same call sites)
  ...
  - Remaining: CashFlow batch 2 (Income, IncomeSource, InvestmentAccount, InvestmentSnapshot, Mensais, ReserveBucket, Reserve, Tithe), CashFlow batch 3 (Transfer) + Investment batch (BrokerBreakdown, Credit, Dividend, Navigation, PortfolioAssetSummary, Price, Summary, Transaction — needs `Microsoft.Extensions.Logging.Abstractions` added to `Financial.Investment.Application.csproj`).
