...
- [ ] T050 [P] Add `ILogger`-based use-case entry/success logging alongside the spans added in T029-T031/T033-T034 (logging-audit.md priority 2 — the two land together naturally at the same call sites)
  ...
  - Remaining: final batch — Transfer (CashFlow) + Investment (BrokerBreakdown, Credit, Dividend, Navigation, PortfolioAssetSummary, Price, Summary, Transaction — needs `Microsoft.Extensions.Logging.Abstractions` added to `Financial.Investment.Application.csproj`). 9 service files + 1 csproj; one file over the batch cap, flagged in the PR for reviewer judgment rather than split into a 1-file PR.
