...
**Batch-size exception (added after PR #467's review, see spec.md Assumptions)**: the ~5-file target is for heterogeneous changes. Repeatable, single-pattern work — instrumenting many services with the identical `ITelemetryTracer` span-wrapping shape (T031, T034, and their analogues) — may batch more files per PR, since every file gets the same already-approved change and splitting further adds PR overhead without adding review safety.
...
### Suggested PR 4e onward — extend CashFlow coverage (repeatable, ~1 csproj + services per PR; batch size relaxed per the exception above) [US1]
...
### Suggested PR 4g onward — extend Investment coverage (repeatable; batch size relaxed per the exception above) [US1]

- [ ] T034 [US1] Repeat T033's pattern for the remaining Investment services — depends on T032. All same-pattern instrumentation may land together rather than split into many small PRs.
...
| 4e+ | T030, T031 | 1 csproj (already added) + services; remaining batch may cover all leftover services (batch-size exception) | US1 (repeatable) |
| 4f | T032–T033 | 2 | US1 |
| 4g+ | T034 | services; batch-size exception applies | US1 (repeatable) |
| 4h | T035–T037 | 3 | US1 |
| 4i | T038–T039 | 3 | US1 |
| 4j | T040 | 1 | US1 |
| 5a | T043 | 1 | US3 |
...
- Full service-by-service tracing coverage (4e+/4g+) is intentionally open-ended in this document — each such PR follows the exact pattern established by T029/T033, so it doesn't need its own enumerated task per service. Per the batch-size exception, these PRs are not held to the ~5-file target — batch as many same-pattern services together as makes sense.
