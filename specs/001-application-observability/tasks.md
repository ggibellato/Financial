...
### Suggested PR 5b — Langfuse local overlay (1 file) [US3] — COMPLETE

- [X] T044 [US3] Add a `langfuse` Compose profile to `docker-compose.observability.yml`, referencing Langfuse's official minimal self-hosted stack with ephemeral/local-only volumes — depends on T027
  - Sourced from upstream `langfuse/langfuse` docker-compose.yml (research.md D7): web + worker + postgres + clickhouse + redis + minio, all under `profiles: ["langfuse"]`, trimmed of optional env blocks (SMTP, LLM connections, batch export, cloud object stores). Ephemeral by construction — **no** named volumes, all credentials throwaway local-only values, only port 3000 (Langfuse UI + OTLP ingest) exposed to the host. `LANGFUSE_INIT_*` seeds an org/project/user headlessly so the committed `pk-lf-local`/`sk-lf-local` keys work with zero UI setup.
  - `app` moved to `profiles: ["jaeger", "langfuse"]` with `${OBSERVABILITY_BACKEND:-Jaeger}`/`${OBSERVABILITY_ENDPOINT:-http://jaeger:4317}` indirection: `--profile jaeger up` behaves exactly as before with zero env vars; the langfuse variant sets two env vars (documented in the file header). The old `depends_on: jaeger` was removed — it would break the langfuse profile (jaeger not started), and export failures are silent/retried by design (SC-006).
...
- [X] T046 [US3] Run and record [quickstart.md](./quickstart.md) Scenario C — depends on T043, T044
  - **Run 2026-08-19**: booted the langfuse profile (all 4 infra containers healthy, Langfuse web 4.15.0 `{"status":"OK"}`); seeded keys authenticate against `/api/public/otel/v1/traces` (HTTP 200; wrong keys → 401). Ran the API locally against a **temp copy** of the real data files with only config changed (`Backend=Langfuse`, endpoint, keys), exercised banks/reserve endpoints, then verified in Langfuse's ClickHouse (`events_full`): 3 traces, each with the ASP.NET root span (`GET .../banks`, `GET .../reserve/balances`, `POST .../reserve/withdrawals`) sharing its `trace_id` with the Application span under it (`CashFlow.BankService.GetBanks`, `CashFlow.ReserveService.GetBucketBalances`, `CashFlow.ReserveService.PostWithdrawal`). Same traces as the Jaeger scenario, zero source-code change. Note: Langfuse v4 runs in `events_only` mode — the public REST `/api/public/traces` read endpoint is unavailable; traces are viewed in the UI (login `local@financial.dev`/`financial-local`) or verified in ClickHouse as above. Stack torn down after; nothing persisted (no volumes).
...
- [X] T049 Run `dotnet build --configuration Release`, `dotnet test`, and `docker-compose up` (base file, unchanged) and record results per Constitution Principle VIII
  - **Run 2026-08-19, on `main` at the T050-complete merge (#484) — the feature's final state**:
    - `dotnet build --configuration Release`: **Build succeeded, 0 errors** (26 warnings, all pre-existing `CS8632` nullable-annotation warnings in `Financial.Investment.Domain`/`Integrations/GoogleFinancialSupport`/`Tools/ImportGoogleSpreadSheets`, unrelated to this feature).
    - `dotnet test --configuration Release`: **all 12 test projects green — 2,598 passed, 0 failed, 5 skipped** (the always-skipped live external-API verification tests in `Financial.Investment.Infrastructure.Tests`). Per-project: CashFlow.Domain 232, Investment.Domain 117, CashFlowSpreadsheetImport 201, Investment.Application 276, CashFlow.Application 385, Observability 21, Architecture 8, Presentation 716, Shared.Infrastructure 41, CashFlow.Infrastructure 73, Investment.Infrastructure 238(+5 skipped), Api 290.
    - `docker compose up --build -d` (base file only, unchanged, no observability overlay): image built (including the React SPA build inside the Dockerfile), container started cleanly with port 8080 verified free beforehand. `GET /api/v1/financial/banks` → 200 `application/json`; `GET /` → 200 `text/html` (SPA); `GET /api/v1/financial/sync-status` → both contexts `Idle`. Container logs contain **zero** error/exception/OTLP/observability entries — the shipped default `Observability:Enabled=false` is fully silent, per Constitution Principle VIII and US2. Only read endpoints were exercised (no data mutations against the mounted live `./data`); stack torn down with `docker compose down` after.
- [X] T050 [P] Add `ILogger`-based use-case entry/success logging alongside the spans added in T029-T031/T033-T034 (logging-audit.md priority 2 — the two land together naturally at the same call sites)
  - **Pattern (established in batch 1)**: required `ILogger<T>` constructor dependency (mirrors `CardStatementService`, the one pre-existing precedent); entry logged once inside the per-service `StartSpan` helper (`"{Operation} started"` — covers every method with one line); success logged next to each `OperationResult=Success` attribute (`"{Operation} completed"`). Only the operation name is ever logged — no DTO fields, descriptions, or values (FR-014). Failures deliberately not logged here: they propagate to the API middleware (T040) or WPF catches (T052), which already log them — logging in Application too would duplicate every failure.
  - **Batch 1 (PR 6d) — done**: AnnualSummary, BalanceAdjustment, Bank, CardStatement (already had the logger; only log lines added), Category, ControleMae, CreditCard, Expense — 8 services. Tests: `Financial.CashFlow.Application.Tests` 376/376 (was 368) — `Constructor_WithNullLogger_Throws` per new-logger service (7) plus a representative `AddExpenseAsync_LogsEntryAndSuccess_WithOperationNameOnly` asserting entry+success messages and that the expense's description never appears in any log entry.
  - **Batch 2 (PR 6e) — done**: Income, IncomeSource, InvestmentAccount, InvestmentSnapshot, Mensais, ReserveBucket, Reserve, Tithe — 8 services, same pattern. Tests: `Financial.CashFlow.Application.Tests` 384/384 (was 376) — `Constructor_WithNullLogger_Throws` per service (8 new).
  - **Final batch (PR 6f) — done, completing T050**: Transfer (CashFlow) + all 8 Investment services (BrokerBreakdown, Credit, Dividend, Navigation, PortfolioAssetSummary, Price, Summary, Transaction), plus `Microsoft.Extensions.Logging.Abstractions` added to `Financial.Investment.Application.csproj`. 9 service files + 1 csproj — one file over the 8-file batch cap, flagged in the PR body for reviewer judgment rather than splitting a single-file PR off. Tests: `Financial.CashFlow.Application.Tests` 385/385 (+1 Transfer null-logger test), `Financial.Investment.Application.Tests` 276/276 (was 269, +7 null-logger tests; PriceService lives in Infrastructure.Tests whose heavier suite historically has no ctor-null tests — kept symmetric). All 26 instrumented services across both bounded contexts now log use-case entry/success alongside their spans.
- [X] T051 [P] Add retry/fallback logging in `Financial.Shared.Infrastructure/Resilience/TransientRetryPolicy.cs` and `Financial.Investment.Infrastructure/Services/FallbackFinanceService.cs` (logging-audit.md priority 3)
  - **Deviation**: `TransientRetryPolicy.cs` itself needed no change — it already accepted an `Action<string>? logger` callback that its only caller (`DebouncedJsonStorage.SaveNowAsync`) never supplied. The fix threads an optional `ILogger` through the same chain the tracer already uses (DebouncedJsonStorage → JsonStorageFactory → both repository factories → both Infrastructure DI extensions, 6 files) and supplies the callback; a terminal save failure now also logs an error (`error.type` only — document content/messages never reach the log stream). `FallbackFinanceService` gained a required `ILogger<T>`: warning when the fallback engages (ticker + primary `error.type` — tickers are public market symbols, not personal financial data), info when it succeeds, error when both providers fail. 7 production files — within the batch exception (same threading pattern as PR 4b's tracer).
  - **Tests (not counted)**: `Financial.Shared.Infrastructure.Tests` 41/41 (was 39) — retry logs a warning with `Retry 1/5` + exception type then no error on success; exhausted retries log one error, never the document body. `Financial.Investment.Infrastructure.Tests` 243/243 (was 241) — fallback engagement logs ticker + error type but never the provider's message; primary success logs nothing.
- [X] T052 [P] Instrument the remaining high-value WPF `catch` blocks identified in `logging-audit.md` (`ReservaViewModel`, `ControleMaeViewModel`, `MensaisViewModel`) with `ILogger`, following the pattern established in T036
  - Each ViewModel gained a required `ILogger<T>` (constructor DI, wired in `App.xaml.cs` — 4 production files). 9 catch blocks instrumented: error-surfacing catches (`Error`/`DeleteError`/`ResetError`/`DeleteMovementError = ex.Message`) log `LogError` with **`error.type` only** (the message shown to the user may embed bucket names/balances/bill values — FR-014 keeps it out of the log stream); the two silent-degrade catches (`TryGetReserveBuckets`, `RefreshTotalsAsync`) now log `LogWarning` instead of swallowing invisibly. The overdraft-confirmation catch in `ReservaViewModel` was deliberately left unlogged — it's control flow (a confirm dialog), not a failure. `ExecuteSaveAsync`-based save commands remain covered by their `setError` surface + T036's span pattern, unchanged.
  - **Tests (not counted)**: `Financial.Presentation.Tests` 716/716 (was 713) — one logging test per ViewModel asserting the error type is logged and the exception message (with an embedded value) is not. Assertions use `AllSatisfy` over all recorded entries because each constructor fires a background refresh that can hit the same failing stub, recording duplicates. Verified with a live WPF smoke launch (temp data copies) — clean startup with the three new DI dependencies.
