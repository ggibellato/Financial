...
The configuration surface `Financial.Api`, `Financial.App`, and `docker-compose.yml` MUST agree on, read exclusively by `Integrations/Observability` (Decision D8 — no other project reads these keys). Follows the existing `appsettings`/environment-variable convention already used for `Investment:Repository:Provider` / `CashFlow:Repository:Provider` (FR-013).
