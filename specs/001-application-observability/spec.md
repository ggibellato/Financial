...
- **FR-005**: The system MUST treat OpenTelemetry instrumentation and export as an infrastructure/integration concern, isolated from Domain and Application business logic, consistent with how existing external integrations (e.g. Google Drive) are isolated in the current architecture.
- **FR-006**: Business logic in the Domain and Application layers MUST NOT take a direct compile-time dependency on OpenTelemetry or any specific observability backend's SDK.
