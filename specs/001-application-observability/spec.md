...
- The `Integrations/Observability` project (FR-005/FR-006) is a new .NET project added to `Financial.slnx`, following the same shape as `Integrations/GoogleFinancialSupport`: it depends on Application-layer interfaces (to know what to implement) and is referenced only by Presentation composition roots (`Financial.Api`, `Financial.App`) for DI registration — never by Domain, Application, or any other Infrastructure project.
