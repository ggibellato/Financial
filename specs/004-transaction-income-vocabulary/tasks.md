...
- [X] T001 Verify baseline is green on branch `004-transaction-income-vocabulary`: `dotnet build --configuration Release` and `dotnet test --settings coverlet.runsettings --results-directory TestResults` both pass before any change in this feature begins
...
- [X] T002 [P] Create `TransactionTypeEffects` in `Financial.Investment.Domain/Rules/TransactionTypeEffects.cs`: `QuantityEffect` enum (`Increase, Decrease, None`), `CashEffect` enum (`In, Out, None`), `TransactionTypeEffect` record, and `TransactionTypeEffects.For(Transaction.TransactionType)` mapping every type per data-model.md's table
- [X] T003 Widen `Financial.Investment.Domain/Entities/Transaction.cs`: extend `TransactionType` enum with `Fee, Redemption, TransferIn, TransferOut, CapitalCall, ReturnOfCapital`; relax `ValidateQuantity`/`ValidateUnitPrice` to require `> 0` only when `TransactionTypeEffects.For(type).Quantity != QuantityEffect.None`, else require exactly `0`; add `Withheld` property (floored at 0, same pattern as `Fees`); replace `TotalPrice` with a type-aware `NetCash` computed property per research.md #2 (`CashEffect.Out` → `-(Gross+Fees+Withheld)`, `.In` → `Gross-Fees-Withheld`, `.None` → `-Fees`) (depends on T002)
- [X] T004 Generalize `Apply`/`Recompute` in `Financial.Investment.Domain/Entities/Transactions.cs` per research.md #1: `QuantityEffect.Increase` types feed `AveragePrice` via `Gross+Fees` the way `Buy` does today; `QuantityEffect.Decrease` + `CashEffect != None` types (Sell, Redemption) realize gain/loss against `NetCash` proceeds; `QuantityEffect.Decrease` + `CashEffect.None` (TransferOut) realizes zero gain/loss, reducing quantity at the existing `AveragePrice`; `QuantityEffect.None` types have no effect on `AveragePrice`/`RealizedCapitalGain` (depends on T002, T003)
- [X] T005 [P] Extend `Financial.Investment.Domain/Rules/SaleCoverageRule.cs`'s `FindFirstUncoveredSale` to treat any `QuantityEffect.Decrease` type as a covered "sale" (not only `Sell`), so `Redemption`/`TransferOut` share the oversell-refusal rule per FR-004 (depends on T002)
- [X] T006 [P] Extend the tie-break in `Financial.Investment.Domain/Rules/TransactionReplayOrder.cs`'s `Sort`/`IsInOrder` from `Type == Sell` to `TransactionTypeEffects.For(type).Quantity == QuantityEffect.Decrease` (depends on T002)
- [X] T007 Fix `Financial.Investment.Domain/Rules/AssetTotalsCalculator.cs`'s `CalculateTotals` per research.md #6a: bucket by `CashEffect` (`.Out` → `totalBought`, `.In` → `totalSold`, `.None` → neither) instead of `Type == Buy ? ... : ...` — the current `else` branch would otherwise silently misclassify every new type as a sale (depends on T002, T003)
- [X] T008 Exclude `Transaction.NetCash` from JSON serialization in `Financial.Investment.Infrastructure/Persistence/InvestmentTypeInfoResolver.cs`'s `ExcludedProperties` set, replacing the existing `Transaction.TotalPrice` entry (depends on T003)
- [X] T009 [P] Create `Tests/Financial.Investment.Domain.Tests/Domain/TransactionTypeEffectsTests.cs`: one case per `TransactionType` asserting its declared `QuantityEffect`/`CashEffect` (depends on T002)
- [X] T010 [P] Extend `Tests/Financial.Investment.Domain.Tests/Domain/TransactionTests.cs` for the widened enum, the relaxed Quantity/UnitPrice validation per type, and `NetCash` (replacing every `TotalPrice` assertion) (depends on T003)
- [X] T011 [P] Extend `Tests/Financial.Investment.Domain.Tests/Domain/TransactionsTests.cs` for average-price/realized-gain behavior of every new type, including the zero-gain-loss `TransferOut` case (depends on T004)
- [X] T012 [P] Extend `Tests/Financial.Investment.Domain.Tests/Domain/SaleCoverageRuleTests.cs` for `Redemption`/`TransferOut` oversell coverage (depends on T005)
- [X] T013 [P] Extend `Tests/Financial.Investment.Domain.Tests/Domain/TransactionReplayOrderTests.cs` for the widened tie-break (depends on T006)
- [X] T014 [P] Extend `Tests/Financial.Investment.Domain.Tests/Domain/AssetTotalsCalculatorTests.cs` for the `CashEffect`-based bucketing, including a `TransferIn`/`TransferOut` case landing in neither bucket (depends on T007)
- [X] T015 [P] Extend `Tests/Financial.Investment.Domain.Tests/Domain/AssetTests.cs`'s `RecordTransaction`/`ReviseTransaction`/`RetractTransaction` tests against the new types, including the oversell-refusal message for `Redemption`/`TransferOut` (depends on T004, T005)
...
- [X] T026 [US2] Widen `Financial.Investment.Domain/Entities/Credit.cs`: rename `CreditType.Rent` to `SecuritiesLendingIncome`, add `Coupon`; relax `ValidateValue` from "invalid if `<= 0`" to "invalid only if `== 0`" (permits a negative correction, FR-015); add `Withheld` (same floor-at-magnitude posture as `Value`'s sign); add `NetAmount` computed property (`Value - Withheld`)
- [X] T027 [P] [US2] Exclude `Credit.NetAmount` from JSON serialization in `Financial.Investment.Infrastructure/Persistence/InvestmentTypeInfoResolver.cs`'s `ExcludedProperties` set (depends on T026)
- [X] T028 **Superseded after real-data verification (research.md #6, second entry)** — the standalone `Tools/InvestmentTransactionIncomeVocabularyMigration` console project this task created was deleted. Its data-file path/backup-then-rewrite responsibilities moved to an on-the-fly document-version upgrade run by the serializer itself on every load, so no separate tool or manual run is ever needed for this or a future stored-vocabulary rename.
- [X] T029 **Superseded after real-data verification** — the raw-JSON `"Rent"` → `"SecuritiesLendingIncome"` rewrite this task implemented as a standalone migrator now lives as `InvestmentDataMigrations.Apply(root, fromVersion)` in `Financial.Investment.Infrastructure/Persistence/InvestmentDataMigrations.cs`, invoked by `InvestmentSerializerAdapter.Deserialize` for any document below `InvestmentDataMigrations.CurrentVersion` (2) — same JsonNode tree-walk, called from the load path instead of a `Program.cs`.
- [X] T030 **Superseded after real-data verification** — no `Program.cs`/console entry point exists anymore; `InvestmentSerializerAdapter.Serialize` stamps every write with `"Version": InvestmentDataMigrations.CurrentVersion`, and `Deserialize` reads `"Version"` (missing = `1`) before applying migrations, replacing the tool's role entirely.
- [X] T031 **Superseded during implementation** — research.md #4 was corrected: the existing `BuildWithCredits`/`BuildWithoutCredits` (using `c.Value`) stay exactly as the **Gross** series; `c.NetAmount` is introduced only in the new Net-of-tax series added by T041, not by editing the existing methods. No code change was needed for this task; `Credit.Value`/`Credit.NetAmount` (T026) were already sufficient once T041 lands.
- [X] T032 [P] [US2] Update `Financial.Investment.Application/DTOs/CreditDTO.cs`: add `Withheld`, `NetAmount` (depends on T026)
- [X] T033 [P] [US2] Update `Financial.Investment.Application/DTOs/CreditCreateDTO.cs` and `CreditUpdateDTO.cs`: add `Withheld` (depends on T026)
- [X] T034 [US2] Update `Financial.Investment.Application/Services/NavigationMapper.cs`'s `MapCredit` to map `Withheld`/`NetAmount` (depends on T032)

### Tests for User Story 2

- [X] T035 **Superseded after real-data verification** — the standalone migration-tool test project (`Tests/Financial.InvestmentTransactionIncomeVocabularyMigration.Tests/`) was deleted along with the tool. Its coverage moved into `Tests/Financial.Investment.Infrastructure.Tests/Persistence/InvestmentSerializerAdapterTests.cs`: `Deserialize_NoVersionProperty_TreatsAsVersion1AndRewritesLegacyRentCreditType`, `Deserialize_Version1_RewritesLegacyRentCreditType`, `Deserialize_CurrentVersion_DoesNotReapplyLegacyRentMigration`, and `Serialize_WritesCurrentVersion`.
...
- [X] T040 **Superseded after real-data verification** — no separate migration run is needed: the real `data/data-investment.json` (already rewritten by the now-deleted tool during earlier verification, and carrying no `"Version"` key) is read as version 1 on the next process start, the rename step is a no-op since nothing left matches `"Rent"`, and the very next save stamps it `"Version": 2`. A normal process restart (API and/or `Financial.App`) is sufficient — see `quickstart.md` §2.
...
- **User Story 2 (Phase 4)**: Depends on Foundational only — `Credit` widening (T026) is independent of US1's `Transaction` work; sequenced after it only because the spec orders it P2.
- **User Story 3 (Phase 5)**: Depends on User Story 1 (`NetCash`, T020) **and** User Story 2 (`NetAmount`, T026) — it builds the Gross/Net split directly on top of both.
