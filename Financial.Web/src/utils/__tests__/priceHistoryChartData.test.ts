...
describe('buildChartData', () => {
  it('combines_price_history_and_transactions_sorted_by_date', () => {
  ...
  it('maps_buy_transactions_to_buyPrice_using_unit_price', () => {
    const points = buildChartData([], [BUY_TRANSACTION])
    expect(points).toEqual([{ x: expect.any(Number), date: '01/02/2024', buyPrice: 90, sellPrice: undefined }])
  })

  it('maps_sell_transactions_to_sellPrice_using_unit_price', () => {
    const points = buildChartData([], [SELL_TRANSACTION])
    expect(points).toEqual([{ x: expect.any(Number), date: '10/02/2024', buyPrice: undefined, sellPrice: 130 }])
  })

  it('maps_price_history_entries_to_price_and_isManual', () => {
    const points = buildChartData([MANUAL_ENTRY], [])
    expect(points).toEqual([{ x: expect.any(Number), date: '15/03/2024', price: 120.5, isManual: true }])
  })
})
