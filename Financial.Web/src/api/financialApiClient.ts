...
export function createFinancialApiClient(options: FinancialApiClientOptions = {}): FinancialApiClient {
  ...
  return {
    ...
    setReportingCurrencyEnabled: (requestBody) =>
      request<ReportingCurrencySettingDto>('/reporting-currency/enabled', { method: 'PUT', body: JSON.stringify(requestBody) }),
    ...
    createBank: (requestBody) =>
      request<BankDto>('/banks', { method: 'POST', body: JSON.stringify(requestBody) }),
    updateBank: (id, requestBody) =>
      request<BankDto>(`/banks/${encodeURIComponent(id)}`, {
        method: 'PUT',
        body: JSON.stringify(requestBody),
      }),
    ...
    getCategoryTotalsAnnualForYear: (year) =>
      request<CategoryTotalsAnnualDto>(`/annual-summary/${year}/category-totals`),
    getInvestmentAnnualResultForYear: (year) =>
