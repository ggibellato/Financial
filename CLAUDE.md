...
green (100%) / yellow (95–99.99%) / amber (90–94.99%) / red (<90%) for both the
`backend` and `web` jobs (the `Check coverage threshold` step, reading
`CoverageReport/Summary.json`'s `summary.linecoverage` on the backend side and
